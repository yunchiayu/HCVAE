...
class LinearRecontruct(nn.Module):
    """
    Maps a small set of centroids E_q of shape (B, pn, C)
    back to a full-resolution flattened feature map of shape (B, H*W, C)
    via a learnable linear mapping.
    """
    def __init__(self, pn: int, hw: int):
        """
        Args:
            pn: number of centroids (the second dim of the input)
            hw: H * W, the desired number of output positions
        """
        super().__init__()
        self.pn = pn
        self.hw = hw
        # maps from pn → hw, shared across all C channels
        self.linear = nn.Linear(pn, hw, bias=False)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """
        Args:
            x: tensor of shape (B, pn, C)
        Returns:
            out: tensor of shape (B, hw, C)
        """
        B, pn, C = x.shape
        assert pn == self.pn, f"Expected pn={self.pn}, got {pn}"
       
        x = x.permute(0, 2, 1)   # (B, pn, C) -> (B, C, pn)
        x = self.linear(x)       # apply linear pn->hw on the last dim: (B, C, hw)
        out = x.permute(0, 2, 1) # (B, C, hw) ->  (B, hw, C)
        return out

# class MLPReconstruct_Ori(nn.Module):
#     def __init__(self, pn, hw, hidden_dim=128):
#         super().__init__()
#         self.net = nn.Sequential(
#             nn.Linear(pn, hidden_dim, bias=True),
#             nn.ReLU(inplace=True),
#             nn.Linear(hidden_dim, hw, bias=False)
#         )
#     def forward(self, x):
#         # x: (B, pn, C)
#         B, pn, C = x.shape
#         x = x.permute(0,2,1)      # -> (B, C, pn)
#         x = self.net(x)           # -> (B, C, hw)
#         return x.permute(0,2,1)   # -> (B, hw, C)

class AttentionReconstruct(nn.Module):
    def __init__(self, e_dim: int):
        ...
        # initialize output projection to identity for stable initial reconstruction
        # init.eye_(self.out.weight)
        # init.zeros_(self.out.bias)

    def forward(self, flat: torch.Tensor, E_q: torch.Tensor) -> torch.Tensor:
        """
        Args:
            flat: Tensor of shape (B, H*W, C) -- per-pixel features, C ==e_dim
            E_q : Tensor of shape (B, pn, C)   -- selected centroids
        Returns:
            z_q_flat: Tensor of shape (B, H*W, C)
        """
        # flat: (B, HW, C), E_q: (B, pn, C)
        B, HW, C = flat.shape
        _, pn, _ = E_q.shape

        Q = self.to_q(flat)          # (B, HW, C)
        K = self.to_k(E_q)           # (B, pn, C)
        # # L2-normalize queries and keys for stable similarity scores
        # Q = F.normalize(Q, dim=-1)
        # K = F.normalize(K, dim=-1)
        # V = self.to_v(E_q)           # (B, pn, C)

        # compute attention
        scale = 1.0 / math.sqrt(Q.shape[-1])
        attn_logits = torch.einsum('bid,bjd->bij', Q, K) * scale  # (B, HW, pn)
        attn = F.softmax(attn_logits, dim=-1)                     # (B, HW, pn)

        # weighted sum of values
        z_q_flat = torch.einsum('bij,bjd->bid', attn, E_q)        # (B, HW, C)
        # z_q_flat = self.out(z)

        return z_q_flat
    

class HierClusterVectorQuantizerEMA3(nn.Module):
    """
    ...
    def __init__(
        ...
        # Reconstruction modules for each scale
        self.reconstructor_list = nn.ModuleList([
            AttentionReconstruct(e_dim=self.e_dim)
            for _ in self.v_cluster_nums
        ])
    ...
    def forward(self, z: torch.Tensor):
        ...
        for si, pn in enumerate(self.v_cluster_nums):
            if pn == 1:
                ...
                z_q_flat_ground_truth = torch.einsum('bip,bpc->bic', probs_b, E_q)  # (B, H*W, C)

                z_q_flat = self.reconstructor_list[si](flat.detach(), E_q)          
                # (B, H*W, C), (B, pn, C) -> (B,  H*W, C)
                ...
                # Feature reconstruction loss between ground truth and predicted flat features
                L_reconstruct = F.mse_loss(z_q_flat_ground_truth.detach(), z_q_flat)
                total_vq_loss += L_reconstruct
