...
class ClusterAttention(nn.Module):
    """
    Self-attention over cluster embeddings.
    """
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """
        Args:
            x: cluster embeddings of shape (B, P, C)
        Returns:
            attended embeddings of shape (B, P, C)
        """
        # Self-attend: query, key, value all = x
        attn_out, _ = self.attn(x, x, x)
        return attn_out



class HierClusterVectorQuantizerEMA2(nn.Module):
    """
    ...
    def __init__(
        ...
        self.num_scales = len(self.v_patch_nums)
        ...
        # Cluster embedding attention
        self.cluster_attn = ClusterAttention(embed_dim=e_dim, num_heads=4)
    ...
    def forward(self, z: torch.Tensor):
        ...
        for si, p in enumerate(self.v_patch_nums):
            pn = p * p
            if pn == 1:
                ...
                # Refine cluster embeddings with self-attention
                E_q = self.cluster_attn(E_q)  # (B, pn, C)

                # Reshape cluster embeddings into spatial grid
                tilde_z_s = E_q.view(B, p, p, C)                       # (B, p, p, C)
                tilde_z_s = tilde_z_s.permute(0, 3, 1, 2)              # (B, C, p, p)
                # Upsample to input resolution
                z_ori_scale = F.interpolate(
                    tilde_z_s, size=(H, W), mode='bicubic'
                ).contiguous()                                         # (B, C, H, W)
