...
class ResidualVectorQuantizerEMA(nn.Module):
    """
    Residual Vector Quantizer with EMA codebook update, as used in VQ-VAE.
    ...
    def forward(self, z: torch.Tensor):
        """
        Residual residual vector quantization with EMA codebook update.
        ...
        for _ in range(self.depth): # from small to large

            z_permute = z_rest.permute(0, 2, 3, 1) # -> (B, H, W, C)

            # Flatten for codebook search
            z_flat = z_permute.reshape(-1, C)     # -> (B * H * W, C)

            # Codebook search: L2 distance = |z-e|^2 = |z|^2 + |e|^2 - 2*|z*e|
            square_sum = torch.sum(z_flat ** 2, dim=1, keepdim=True) + torch.sum(self.embedding**2, dim=1) # (B*H*W, n_e)
            dot_product = z_flat @ self.embedding.permute(1, 0) # (B*H*W, C) @ (C, n_e) = (B*H*W, n_e)
            ...
            z_k = self.embedding[min_encoding_indices].view(z_permute.shape) # (B*H*W, C) -> (B, H, W, C)
            residual_z = z_k.permute(0, 3, 1, 2).contiguous() # -> (B, C, H, W)
            ...
            one_hot = F.one_hot(min_encoding_indices, num_classes=self.n_e).type(z_flat.dtype)  # (B*pn*pn, n_e), B*pn*pn rows, and each row is one-hot vector
            counts = one_hot.sum(0) # (n_e, )
            sums   = one_hot.permute(1, 0) @ z_flat # (n_e, B*pn*pn)
            total_counts += counts
        ...
        mean_vq_loss = total_vq_loss / self.depth
